let cart = JSON.parse(localStorage.getItem('amulCart')) || [];
let deliveryData = {};
let promoDiscount = 0;
let isExpress = false;

const promoCodes = {
    "AMUL50": { type: "flat", value: 50, desc: "₹50 off" },
...
if (cart.length === 0 && !localStorage.getItem('amulOrderPlaced')) {
...
function renderSummary(targetId) {
    ...
    container.innerHTML = cart.map(c => `
        <div class="summary-item">
            <span class="summary-item-emoji">${c.emoji}</span>
            <div class="summary-item-info">
                <p class="summary-item-name">${c.name}</p>
                <p class="summary-item-qty">Qty: ${c.qty} x ₹${c.price}</p>
            </div>
            <span class="summary-item-price">₹${c.price * c.qty}</span>
        </div>
    `).join('');
}
...
function calculateTotals() {
    ...
    // Apply delivery promo
    const appliedPromo = document.getElementById('promoInput')?.value?.toUpperCase();
    if (appliedPromo && promoCodes[appliedPromo]?.type === 'delivery') {
        deliveryFee = 0;
    }

    if (appliedPromo && promoCodes[appliedPromo]?.type === 'flat') {
        discount = promoCodes[appliedPromo].value;
    } else if (appliedPromo && promoCodes[appliedPromo]?.type === 'percent') {
        discount = Math.min(subtotal * promoCodes[appliedPromo].value / 100, promoCodes[appliedPromo].max || Infinity);
    }
...
function updateTotals() {
    ...
    ['subtotal','subtotal2'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.textContent = '₹' + t.subtotal;
    });
    ['deliveryFee','deliveryFee2'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.textContent = t.deliveryFee === 0 ? 'FREE' : '₹' + t.deliveryFee;
    });
    ['gst','gst2'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.textContent = '₹' + t.gst;
    });
    ['finalTotal','finalTotal2'].forEach(id => {
        const el = document.getElementById(id);
    ...
    if (t.discount > 0) {
        ...
        ['discountAmt','discountAmt2'].forEach(id => {
            const el = document.getElementById(id);
            if (el) el.textContent = '-₹' + t.discount;
        });
    }
}
...
function renderDateOptions() {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        dates.push({ label, dateStr, isToday: i === 0 });
    }

    container.innerHTML = dates.map((d, i) => `
        <label class="date-card ${i === 0 ? 'selected' : ''}">
            <input type="radio" name="date" value="${d.dateStr}" ${i === 0 ? 'checked' : ''}>
            <span>${d.label}</span>
        </label>
    `).join('');
...
document.getElementById('applyPromo')?.addEventListener('click', () => {
    ...
    if (!code) { msg.textContent = 'Please enter a promo code'; msg.style.color = '#e63946'; return; }

    if (promoCodes[code]) {
        promoDiscount = 0;
        msg.textContent = `Code applied! ${promoCodes[code].desc}`;
        msg.style.color = '#2d6a4f';
        applied.style.display = 'flex';
        applied.querySelector('span').textContent = code + ' APPLIED';
    } else {
        promoDiscount = 0;
...
function validatePaymentForm() {
    ...
    if (method === 'card') {
        ...
        return num?.length >= 13 && name && exp?.length === 5 && cvv?.length === 3;
    }
    if (method === 'netbanking') {
        return document.querySelector('input[name="bank"]:checked');
    }
    return true;
}
...
document.getElementById('toPayment')?.addEventListener('click', () => {
    ...
    deliveryData = {
        ...
        date: document.querySelector('input[name="date"]:checked')?.value || 'today'
    };
...
document.getElementById('placeOrder')?.addEventListener('click', () => {
    ...
    const orderId = 'AMUL' + Date.now().toString(36).toUpperCase() + Math.random().toString(36).substring(2,6).toUpperCase();
    ...
    // Clear cart
    localStorage.removeItem('amulCart');
    localStorage.setItem('amulOrderPlaced', 'true');
